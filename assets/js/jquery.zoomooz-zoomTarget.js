...
(function($) {
  ...
  function setupCssStyles() {
    ...
    function setupSelectionCss(enabled) {
      var selectionString =
        "-webkit-touch-callout: " + (enabled ? "default" : "none") + ";";
      helpers.forEachPrefix(function(prefix) {
        selectionString +=
          prefix + "user-select:" + (enabled ? "text" : "none") + ";";
      }, true);
    ...
    style.innerHTML =
      ".zoomTarget{" +
      setupSelectionCss(false) +
      "}" +
      ".zoomTarget:hover{cursor:pointer!important;}" +
      ".zoomNotClickable{" +
      setupSelectionCss(true) +
      "}" +
  ...
  function videoChecker(clickTarget) {
    var target = clickTarget.children();
    var targetchecked = target.is("video");
    if (targetchecked) {
      pauseVideo();
      target.get(0).play();
    } else {
